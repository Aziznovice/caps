   ...
   public $updated;
	...
	public function getUsersListing(){		
		...
		while ($user = $result->fetch_assoc()) { 				
			...
			$rows[] = $type;			
			$rows[] = $status;				
			$rows[] = '<a href="edit_users.php?id='.$user["id"].'" class="btn btn-warning btn-xs update">Edit</a>';
	...
	public function insert(){
		...
		if($this->email && $this->password) {
			...
			// Create variables for md5($this->password) and $this->deleted
			$passwordHash = md5($this->password);
			$deleted = $this->deleted;
	
			$stmt->bind_param("ssssii", $this->first_name, $this->last_name, $this->email, $passwordHash, $this->type, $deleted);
